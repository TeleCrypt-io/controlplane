...
func TestMigrate(t *testing.T) {
	...
	defer pool.Close()
	if _, err := pool.Exec(ctx, `DROP TABLE IF EXISTS ownership, pending_claims, verified, locker_state, schema_migrations`); err != nil {
	...
	for _, table := range []string{"ownership", "pending_claims", "teams", "seats", "billing_environment_guard"} {
		...
		if exists {
			t.Errorf("public migration must not create %q", table)
		}
	}
	if err := Migrate(ctx, pool); err != nil {

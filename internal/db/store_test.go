...
func testStore(t *testing.T) (*Store, context.Context) {
	...
	if _, err := pool.Exec(ctx, `DROP SCHEMA IF EXISTS cashier CASCADE; DROP TABLE IF EXISTS ownership, pending_claims, verified, locker_state, schema_migrations`); err != nil {
		t.Fatalf("reset database: %v", err)
	}
	if err := Migrate(ctx, pool); err != nil {
		t.Fatalf("Migrate: %v", err)
	}
	if _, err := pool.Exec(ctx, `CREATE SCHEMA cashier; CREATE TABLE cashier.billing_environment_guard (singleton BOOLEAN PRIMARY KEY, billing_env TEXT NOT NULL, matrix_deployment_id TEXT NOT NULL); CREATE TABLE cashier.billing_verification_grants (mxid TEXT PRIMARY KEY)`); err != nil {
...
func TestStoreReadsButDoesNotCreatePrivateCashierState(t *testing.T) {
	...
	pool := store.pool
	if _, err := pool.Exec(ctx, `INSERT INTO cashier.billing_environment_guard (singleton, billing_env, matrix_deployment_id) VALUES (TRUE, 'test', 'production')`); err != nil {
...
func TestStoreCombinesManualAndPrivateCashierVerificationGrants(t *testing.T) {
	store, ctx := testStore(t)
	if _, err := store.pool.Exec(ctx, `INSERT INTO verified (mxid) VALUES ('@manual:telecrypt.io'); INSERT INTO cashier.billing_verification_grants (mxid) VALUES ('@billing:telecrypt.io')`); err != nil {
	...
	if !verified["@manual:telecrypt.io"] || !verified["@billing:telecrypt.io"] {

...
func LoadLocker() (*LockerConfig, error) {
    ...
    for _, mxid := range getenvList("EXCLUDE_MXIDS") { exclude[mxid] = true }
    c := &LockerConfig{
        LogLevel: getenvDefault("LOG_LEVEL", "info"),
    ...
    } { if req.value == "" { missing = append(missing, req.name) } }
    if c.SMTPHost != "" && c.SMTPFrom == "" { missing = append(missing, "SMTP_FROM (required once SMTP_HOST is set)") }
    if len(missing) != 0 { return nil, fmt.Errorf("missing required env vars: %s", strings.Join(missing, ", ")) }
    if c.BillingEnv != "test" && c.BillingEnv != "production" { return nil, fmt.Errorf("BILLING_ENV must be exactly test or production") }
    return c, nil
}
...
func LoadSteward() (*StewardConfig, error) {
    ...
    } { if req.value == "" { return nil, fmt.Errorf("missing required env var: %s", req.name) } }
    if c.BillingEnv != "test" && c.BillingEnv != "production" { return nil, fmt.Errorf("BILLING_ENV must be exactly test or production") }
    if err := validatePublicHTTPSURL(c.Homeserver, "HOMESERVER"); err != nil { return nil, err }
    if err := validatePublicHTTPSURL(c.PlanPublicURL, "PLAN_PUBLIC_URL"); err != nil { return nil, err }
    if err := validateComposeInternalOrigin(c.CashierInternalURL, "cashier", "9011", "CASHIER_INTERNAL_URL"); err != nil { return nil, err }
    if err := validateComposeInternalOrigin(c.MASBaseURL, "mas", "8080", "MAS_BASE_URL"); err != nil { return nil, err }
    return c, nil
}

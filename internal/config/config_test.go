...
func TestLoadSteward_UsesPrivateCashierTopology(t *testing.T) {
    ...
    cfg, err := LoadSteward()
    if err != nil { t.Fatalf("LoadSteward: %v", err) }
    if got, want := cfg.ListenAddr, ":9012"; got != want { t.Fatalf("ListenAddr = %q, want %q", got, want) }
}

func TestLoadSteward_RejectsPublicCashierOrigin(t *testing.T) {
    setRequiredStewardEnv(t)
    t.Setenv("CASHIER_INTERNAL_URL", "https://billing.test.telecrypt.io")
    if _, err := LoadSteward(); err == nil { t.Fatal("LoadSteward accepted public Cashier origin") }
}
...
func TestLoadLocker_RejectsUnknownBillingEnvironment(t *testing.T) {
    ...
    if _, err := LoadLocker(); err == nil { t.Fatal("LoadLocker accepted invalid billing environment") }
}
